...
public static class ReflectionCache
{
    ...
    private static bool IsValueTypeWithReferenceFieldsImpl(Type type)
    {
        if (!type.IsValueType)
            return false;
        return type.GetProperties(GetBindingFlags(CacheBehaviour.IncludePrivate))
            .Any(x => x.PropertyType.IsClass);
    }

    public static void CachePrivatePropertiesOfTypes(IEnumerable<Type> types)
    {
        PropertyReader[] GetAllProperties(Type t) =>
            GetPropertiesAndFields(t, CacheBehaviour.IncludePrivate);
    ...
    public static PropertyReader[] GetProperties(object obj)
    {
        ...
        PropertyReader[] getPublicProperties(Type t) =>
            GetPropertiesAndFields(t, CacheBehaviour.PublicOnly);
    ...
    {
        return GetProperties(type, behaviour).Concat(GetFields(type, behaviour)).ToArray();
    }
    ...
    private static IEnumerable<PropertyReader> GetProperties(Type type, CacheBehaviour behaviour)
    {
        ...
        return properties.Select(x => new PropertyReader(x.Name, o => x.GetValue(o, null), type));
    }

    private static IEnumerable<PropertyInfo> RemoveHiddenProperties(
        IEnumerable<PropertyInfo> properties
    )
    {
        return properties.ToLookup(x => x.Name).Select(x => x.First());
    }

    private static IEnumerable<PropertyInfo> ExcludeIndexProperties(
        IEnumerable<PropertyInfo> properties
    )
    {
        return properties.Where(x => !x.GetIndexParameters().Any());
    }

    private static IEnumerable<PropertyInfo> ExcludeSetOnlyProperties(
        IEnumerable<PropertyInfo> properties
    )
    {
        return properties.Where(x => x.GetMethod != null);
    }
    ...
    private static IEnumerable<PropertyReader> GetFields(Type type, CacheBehaviour behaviour)
    {
        ...
        return fields.Select(x => new PropertyReader(x.Name, o => x.GetValue(o), type));
    }
