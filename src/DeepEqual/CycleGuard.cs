...
public class CycleGuard(IComparison inner) : IComparison
{
	...
	)
	{
		...
		{
			if (
				ReferenceEquals(frame.Value1, value1) &&
				ReferenceEquals(frame.Value2, value2)
			)
			{
	...
	private static (ComparisonResult result, IComparisonContext context) ThrowCircularReferenceException(
		ComparisonFrame frame,
		IComparisonContext context,
		object value1,
		object value2
	)
	{
		...
		throw new ObjectGraphCircularReferenceException(message, context.Breadcrumb, value1, value2);
	}
